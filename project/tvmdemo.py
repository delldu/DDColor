...
def compile():
    ...
    if not os.path.exists("output/image_color.so"):
        input1 = torch.randn(SO_B, SO_C, SO_H, SO_W)
        input2 = torch.randn(SO_B, SO_C, SO_H, SO_W)
        todos.tvmod.compile(model, device, [input1, input2], "output/image_color.so")
    todos.model.reset_device()


def predict(gray_files, color_files, output_dir):
    ...
    # # load files
    # image_filenames = todos.data.load_files(gray_files)
    # color_filenames = todos.data.load_files(color_files)
...
if __name__ == "__main__":
    compile()
    predict("images/grey/*.jpg", "images/color/*.jpg", "output/so")
